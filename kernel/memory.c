...
#define KERNEL_MEMORY_SIZE (0x100000 * sizeof(KERNEL_PAGE_TABLE)) // 1M * 8
...
void mapping_init()
{
    ...
    // 将最后一个页表指向页目录自己，方便修改, 0xfffff00
    // fffff000 -> 1111 1111 1111 1111 1111 0000 0000 0000
    // 根据前10位找到页目录表，拿到页表的基地址1(单位页), 因为是用20位表示的，所以基地址是1 >> 3。所以就又回到了0x1000，所以还是页目录
    // 在根据中间10位找到再在页目录找，拿到页表的基地址1(单位页), 因为是用20位表示的，所以基地址是1 >> 3。所以就又回到了0x1000，所以还是页目录
    // 最后根据偏移0, 又回到了0x1000
